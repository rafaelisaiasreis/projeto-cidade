...
public class LoadTableCityFromCSVFile {
  ...
  public void load() {
    ...
    try {
      citiesDTOGroup.parallelStream()
          .forEach(city -> cidadeRepository.saveAndFlush(City.parseDtoToCityObject(city)));
    } catch (Exception ex) {
  ...
  private List<CityCsvFileDTO> csvToBean() {
    try (Reader reader = Files.newBufferedReader(Paths.get(pathToCitiesCsvFile))) {
      CsvToBean<CityCsvFileDTO> csvToBean = new CsvToBeanBuilder<CityCsvFileDTO>(reader)
          .withType(CityCsvFileDTO.class).withIgnoreLeadingWhiteSpace(true).build();
